        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if load_button_rect.collidepoint(event.pos):
                    # Пользователь успешно нажал на кнопку, можно переходить к следующему состоянию
                    waiting_for_button_press = False
                    game_state = STATE_GAME_ACTIVE
                    should_analyze_image = True  # Предполагаем, что изображение было успешно загружено
                    print("Изображение загружается...")
                else:
                    # Нажатие было вне кнопки, игнорируем его
                    print("Неуспешное нажатие, попробуйте снова.")
    ...
    while True:
        ...
        if game_state == STATE_START_SCREEN:
            ...
            while game_state == STATE_GAME_ACTIVE:  # Используем условие для выхода из цикла при смене состояния
                ...
                # Добавленная логика для перехода к начальному экрану при касании шариком нижней границы
                if ball_body.position.y + ball_shape.radius >= screen_height:
                    game_state = STATE_START_SCREEN
                    break  # Опционально: выход из цикла
